...
function Style:ApplyHealthBarStyle(frame, db)
	...
	if BarStyling then
		BarStyling:ApplyHealthBarStyle(frame, frame.healthbar, frame.HealthBarsContainer, db, "pet");
...
function Style:UpdateHealthText(frame, db)
	...
	if UnitFrameStyle then
		UnitFrameStyle:UpdateTextForBar(
			...
			"pet",
			"PET",
			function(template, current, max) return UnitFrameStyle:FormatHealth("pet", template, current, max); end,
			function() return UnitHealth("pet"); end,
			function() return UnitHealthMax("pet"); end
		);
	end
end
...
function Style:UpdatePowerText(frame, db)
	...
	if UnitFrameStyle then
		UnitFrameStyle:UpdateTextForBar(
			...
			"pet",
			"PET",
			function(template, current, max) return UnitFrameStyle:FormatPower("pet", template, current, max); end,
			function() return UnitPower("pet", powerType); end,
			function() return UnitPowerMax("pet", powerType); end
		);
	end
end
...
function Style:ApplyPowerBarStyle(frame, db)
	...
	if BarStyling then
		BarStyling:ApplyPowerBarStyle(frame, frame.PowerBar, db, "pet");
	end
